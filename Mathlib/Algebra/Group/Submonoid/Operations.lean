...
@[to_additive (attr := simp)]
lemma comap_piMap_pi {N : ι → Type*} [Π i, MulOneClass (N i)] {J : Set ι} {S : Π i, Submonoid (N i)}
    (f : Π i, M i →* N i) :
...
@[to_additive (attr := simp)]
lemma _root_.MonoidHom.mrange_piMap {N : ι → Type*} [Π i, MulOneClass (N i)] (f : Π i, M i →* N i) :
