...
instance : MulAction R' (M [⋀^ι]→L[A] N) := wrap_instance%
...
instance addCommMonoid : AddCommMonoid (M [⋀^ι]→L[R] N) := wrap_instance%
...
instance : AddCommGroup (M [⋀^ι]→L[R] N) := wrap_instance%
...
instance [ContinuousAdd N] : DistribMulAction R (M [⋀^ι]→L[A] N) := wrap_instance%
...
instance : Module R (M [⋀^ι]→L[A] N) := wrap_instance%
